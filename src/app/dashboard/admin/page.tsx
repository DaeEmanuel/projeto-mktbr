...
import { CourseVideoManager } from "@/components/course-video-manager";
...
type BannerRow = { id: string; title: string | null; desktop_image_url?: string | null; mobile_image_url?: string | null; image_url?: string | null; redirect_url?: string | null; link_url?: string | null; start_date?: string | null; end_date?: string | null; display_order: number | null; is_active?: boolean | null; active?: boolean | null; created_at?: string | null; updated_at?: string | null };
...
type SubscriptionRow = { user_id: string | null; plan_name: string | null; status: string | null; subscription_plan: string | null; subscription_status: string | null };
...
export default async function AdminDashboardPage() {
  ...
  const [users, courses, books, orders, sales, banners, notifications, subscriptions] = await Promise.all([
    ...
    safeList<BannerRow>(dataClient.from("banners").select("id, title, desktop_image_url, mobile_image_url, image_url, redirect_url, link_url, start_date, end_date, display_order, is_active, active, created_at, updated_at").eq("position", "home").order("display_order", { ascending: true }).order("created_at", { ascending: false }).limit(120)),
    ...
    safeList<SubscriptionRow>(dataClient.from("subscriptions").select("user_id, plan_name, status, subscription_plan, subscription_status").limit(500)),
  ]);
  const subscriptionByUser = new Map<string, SubscriptionRow>(subscriptions.filter((item) => item.user_id).map((item) => [item.user_id as string, item]));
  ...
  return <main className="min-h-screen bg-[#f4f8f3] text-[#061421]"><header className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-5 sm:px-6 lg:px-8"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-[#00a843]">Admin MKTBR Site</p><h1 className="text-3xl font-black">Central administrativa</h1><p className="mt-1 text-sm font-semibold text-slate-600">Gestão de conteúdo, vendas, usuários, banners, notificações e configurações.</p></div><Link href="/meu-painel" className="rounded-lg bg-[#061421] px-4 py-3 text-sm font-black text-white">Meu Painel</Link></div></header><div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:px-8">
    ...
    <section id="usuarios" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm scroll-mt-6"><h2 className="text-2xl font-black">Gestão de Usuários</h2><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="text-slate-500"><tr><th className="py-3">Nome</th><th>E-mail</th><th>Role</th><th>Plano</th><th>Status</th><th>Cadastro</th><th>Acoes</th></tr></thead><tbody>{users.map((item) => <tr key={item.id} className="border-t border-slate-100"><td className="py-3 font-black">{item.name || "-"}</td><td>{item.email}</td><td>{item.role || "user"}</td><td>{subscriptionByUser.get(item.id)?.plan_name || subscriptionByUser.get(item.id)?.subscription_plan || "Sem plano"}</td><td>{subscriptionByUser.get(item.id)?.subscription_status || subscriptionByUser.get(item.id)?.status || "-"}</td><td>{item.created_at ? new Date(item.created_at).toLocaleDateString("pt-BR") : "-"}</td><td><AdminActionButton>Alterar role</AdminActionButton></td></tr>)}</tbody></table>{users.length === 0 ? <EmptyState text="Nenhum usuário listado. Verifique as policies administrativas." /> : null}</div></section>
